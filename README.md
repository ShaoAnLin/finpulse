...
3. **send_messages.py** — Pushes the two features to LINE using the Messaging API push endpoint, then records the delivered articles (including category, source, snippet, published date, and the AI-written feature text) in `state.sqlite3`'s `pushed_news` table
